...
class ArchivedRequestProcessor {
  static newOne () {
    const rp = new ArchivedRequestProcessor()
    rp.init()
    return rp
  }
  ...
  process (data, temporalInfo) {
    if (data.requests) {
      let {requests, webSockets} = data.requests
          ...
          if (reqObj !== null && reqObj !== undefined) {
            if (Array.isArray(reqObj)) {
              ...
              while (j < len2) {
                ...
                temporalInfo.processPageRequest(requests[i].requestId, reqObj[i])
                if (reqObj[j].redirectResponse) {
                  ...
                  temporalInfo.processPageRedirect(requests[i].requestId, reqObj[j].redirectResponse)
                }
                j++
              }
            } else {
              ...
              this.reqChainCount[requests[i].requestId] += 1
              temporalInfo.processPageRequest(requests[i].requestId, reqObj)
              ...
              if (reqObj.redirectResponse) {
                ...
                temporalInfo.processPageRedirect(requests[i].requestId, reqObj.redirectResponse)
              }
            }
          }
          if (resObj !== null && resObj !== undefined) {
            if (Array.isArray(resObj)) {
              ...
              while (j < len2) {
                ...
                temporalInfo.processPageRes(requests[i].requestId, resObj[i])
                j++
              }
            } else {
              if (this.reqChainCount[requests[i].requestId] === undefined) {
              ...
              temporalInfo.processPageRes(requests[i].requestId, resObj)
            }
          }
  ...
  _processARequest (reqObj, aReqTime) {
    ...
    if (reqObj.request && reqObj.request.url !== undefined) {
      try {
        ...
        let o = _.takeRight(this._urlParser.origin.split('.'), 2).join('.')
        if (this.requestOriginCount[o] === undefined) {
          this.requestOriginCount[o] = 0
        }
        this.requestOriginCount[o] += 1
        let h = _.takeRight(this._urlParser.host.split('.'), 2).join('.')
    ...
    if (reqObj.documentURL !== ':') {
      try {
        ...
        du = _.takeRight(du.split('.'), 2).join('.')
      } catch (error) {
...
module.exports = ArchivedRequestProcessor
