...
const RequestProcessor = require('./archivedRequestProcessor')
...
const TemporalInfo = require('./temporalInfo')
const _ = require('lodash')

class ArchiveDumpProcessor {
  constructor (apps) {
    this._apps = apps
  }

  static newOne (apps) {
    return new ArchiveDumpProcessor(apps)
  }
  ...
  process (dumpData, rank) {
    ...
    if (this.temporalInfo === undefined) {
      this.temporalInfo = new TemporalInfo()
      this.temporalInfo.init(url)
    } else {
      this.temporalInfo.init(url)
    }
    this.pageProcessor.process(dumpData)
    this.domProcessor.process(dumpData)
    this.requestProcessor.process(dumpData, this.temporalInfo)
    // console.log(this.temporalInfo._reqTemporalInfo)
...
module.exports = ArchiveDumpProcessor
